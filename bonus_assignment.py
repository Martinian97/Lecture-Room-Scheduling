...
bonus1=0
bonus2=0
print 'Do you wish to assign a particular timeslot to a Professor. Type 1 for yes or 0 for no :',
choice = int(raw_input())
if(choice==1):
	bonus1=1
	print 'Enter the full name of the Professor :',
	wishprof = str(raw_input()).split()[0]
	print 'Make sure that the course belongs to the professor mentioned above. Enter the course code (like CS228) :',
	wishcourse = str(raw_input())
	print 'Make sure that the number of days is <= the number of times the class is scheduled in the week given in input.' 
	print 'Enter the day(s) for which you prefer this timeslot seperated by comma (like Monday,Tuesday,...) :',
	inputdays = str(raw_input())
	inputdays += ',' 
	wishdays = list(map(str,inputdays.split(',')))
	print 'Make sure the duration of the timeslot is the same as specified in the input for the respective course. Enter the timeslot you prefer in HH:mm-HH:mm format (like 08:30-10:00) :',
	wishtime = str(raw_input())

print 'Do you wish to assign a particular timeslot to a batch. Type 1 for yes or 0 for no :',
choice = int(raw_input())
if(choice==1):
	bonus2=1
	print 'Enter the batch no (like 1) :',
	wishbatch = int(raw_input())
	print 'Enter the day(s) for which you prefer this timeslot seperated by comma (like Monday,Tuesday,...) :',
	inputdays = str(raw_input())
	inputdays += ',' 
	wishdays1 = list(map(str,inputdays.split(',')))
	print 'Make sure that there should be some course in input file of this batch of duration = duration of the timeslot you specify. Enter the timeslot you prefer in HH:mm-HH:mm format (like 08:30-10:00) :',
	wishtime1 = str(raw_input())

print 'Calculating based on your preferences'
		...
		for j in Time:
			if(c[1]==float(j)):
				for k in Time[j]:
					...
					code = c[0].split('-')[0]
					if(bonus1==1 and code==wishcourse and k==wishtime and day in wishdays and day not in temp and c[0] not in temp2):
						temp.append(day)
						temp2.append(c[0])
						s.add(D[c[0]][day][k])
		...
		for j in Time:
			if(float(j)==duration):
				for k in Time[j]:
					...
					if(bonus1==1 and c==wishprof and k==wishtime and day in wishdays):
						s.add(F[c][day][k])
		...
		for j in Time:
			if(float(j)==duration):
				for k in Time[j]:
					...
					if(bonus2==1 and c==wishbatch and day in wishdays1 and k==wishtime1):
						s.add(G[c][day][k])
